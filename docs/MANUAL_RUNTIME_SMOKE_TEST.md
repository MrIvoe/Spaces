...
1. Ensure no running instance:
   - tasklist /FI "IMAGENAME eq SimpleFences.exe" /FI "IMAGENAME eq IVOEFences.exe"
2. Build Debug host and tests:
   - C:/Program Files (x86)/Microsoft Visual Studio/18/BuildTools/Common7/IDE/CommonExtensions/Microsoft/CMake/CMake/bin/cmake.exe --build build --config Debug --target HostCoreTests SimpleFences
...
2. Change these values:
   - Appearance -> Visual Modes -> Apply visual mode to all fences
   - Folder Portal -> General -> Default portal mode
   - Fence Organizer -> Large file threshold (MB)
...
2. If no source was assigned, verify fence title indicates needs_source state.
3. Assign a valid source path using host portal setup flow.
4. Confirm fence refreshes and lists source folder items.
...
1. Right-click a fence background and run:
