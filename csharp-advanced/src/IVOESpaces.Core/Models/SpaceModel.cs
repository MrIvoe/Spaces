...
public enum SpaceType { Standard, Portal }
...
public record SpaceModel
{
    ...
    // Per-space visual overrides (null = use global theme)
    public string? BackgroundColorOverride { get; set; }
    public string? TitleColorOverride      { get; set; }
    public string? IconTintColorOverride   { get; set; } // Step 36: Per-space icon tint (ARGB hex)
    ...
    public SpaceSettingsOverrides SettingsOverrides { get; set; } = new();
    ...
    // If TabContainerId is null, this space is standalone.
    // If TabContainerId is set, this space is part of a tabbed container.
    ...
    // Items — not used for Portal spaces (they read from filesystem at runtime)
    public List<SpaceItemModel> Items { get; set; } = new();
}
