...
/// <summary>
/// Step 30: Snap behavior service for magnetic space alignment.
/// Handles snapping to screen edges, other spaces, and monitor center.
/// </summary>
public sealed class SnapService
{
    ...
    /// <summary>
    /// Snap space position and size to nearest targets.
        ...
        Guid excludeSpaceId)
    {
        ...
        if (_settings.SnapToMonitorCenter)
        {
            ...
            double spaceCenterX = snappedX + width / 2.0;
            double spaceCenterY = snappedY + height / 2.0;

            double distCenterX = Math.Abs(spaceCenterX - centerX);
            double distCenterY = Math.Abs(spaceCenterY - centerY);
        ...
        // Step 30: Snap to other spaces
        if (_settings.SnapToOtherSpaces)
        {
            var otherSpaces = GetOtherSpacesOnScreen(screen, excludeSpaceId);
    ...
    /// <summary>
    /// Get list of other spaces on the same screen for snap comparison.
    /// Returns space bounds in screen coordinates.
    /// </summary>
    private List<(double X, double Y, double Width, double Height)> GetOtherSpacesOnScreen(
        System.Windows.Forms.Screen screen, Guid excludeSpaceId)
    {
        ...
        try
        {
            ...
            var spacesOnScreen = allSpaces
                .Where(f => f.Id != excludeSpaceId && f.MonitorDeviceName == screen.DeviceName)
                .ToList();

            foreach (var space in spacesOnScreen)
            {
                var workArea = screen.WorkingArea;

                double x = workArea.X + workArea.Width * space.XFraction;
                double y = workArea.Y + workArea.Height * space.YFraction;
                double w = workArea.Width * space.WidthFraction;
                double h = workArea.Height * space.HeightFraction;
