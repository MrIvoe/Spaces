...
public sealed class WorkspaceOrchestrator
{
    ...
    public async Task<bool> ActivateProfileWorkspaceAsync(SpaceProfileModel profile)
    {
        ...
        WorkspaceLinkService.Instance.SwitchWorkspace(profile, SpaceStateService.Instance.Spaces);
        return true;
    }
                ...
                switch (key)
                {
                    ...
                    case "SpaceOpacity":
                        if (int.TryParse(value, out int opacity))
                            settings.SpaceOpacity = Math.Clamp(opacity, 20, 100);
                        break;
                    ...
                    case "EnableQuickHideMode":
                        ...
                        break;
                    case "StandardSpaceDropMode":
                        settings.StandardSpaceDropMode = value;
                        break;
                }
            }
