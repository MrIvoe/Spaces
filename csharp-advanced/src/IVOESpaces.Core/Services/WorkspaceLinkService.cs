...
public sealed class WorkspaceLinkService
{
    ...
    public WorkspaceSwitchResult SwitchWorkspace(SpaceProfileModel profile, IReadOnlyList<SpaceModel> spaces)
    {
        ...
        var desiredItems = SelectProfileItems(profile, spaces);
    ...
    private static List<SpaceItemModel> SelectProfileItems(SpaceProfileModel profile, IReadOnlyList<SpaceModel> spaces)
    {
        IEnumerable<SpaceModel> chosen = profile.VisibleSpaceIds.Count > 0
            ? spaces.Where(f => profile.VisibleSpaceIds.Contains(f.Id))
            : spaces;

        return chosen
            .Where(f => f.Type == SpaceType.Standard)
    ...
    private static List<WorkspaceEntry> BuildLinkEntries(IReadOnlyList<SpaceItemModel> items)
    {
        var result = new List<WorkspaceEntry>(items.Count);
        var used = new HashSet<string>(StringComparer.OrdinalIgnoreCase);

        foreach (SpaceItemModel item in items)
        {
