...
/// Raises events so the UI layer can auto-capture new items into spaces.
/// </summary>
public sealed class DesktopWatcherService : IDisposable
{
    ...
    /// </summary>
    public List<SpaceItemModel> ScanDesktop()
    {
        var items = new List<SpaceItemModel>();
    ...
    private static void ScanPath(string path, List<SpaceItemModel> items)
    {
        try
        {
    ...
    private void OnCreated(FileSystemWatcher watcher, FileSystemEventArgs e, int generation)
    {
        ...
        if (item != null)
        {
            ...
            SpaceItemReconciliationService.Instance.OnDesktopItemCreated(e.FullPath);
    ...
    private void OnDeleted(FileSystemWatcher watcher, FileSystemEventArgs e, int generation)
    {
        ...
        // Update entity registry and reconcile
        SpaceItemReconciliationService.Instance.OnDesktopItemDeleted(e.FullPath);
    ...
    private void OnRenamed(FileSystemWatcher watcher, RenamedEventArgs e, int generation)
    {
        ...
        // Update entity registry and reconcile
        SpaceItemReconciliationService.Instance.OnDesktopItemRenamed(e.OldFullPath, e.FullPath);
    ...
    private static SpaceItemModel? CreateItemFromPath(string path)
    {
        try
        {
...
{
    public SpaceItemModel Item { get; init; } = null!;
