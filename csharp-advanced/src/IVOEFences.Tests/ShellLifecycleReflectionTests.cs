...
public class ShellLifecycleReflectionTests
{
    ...
    {
        using var manager = new FenceManager();
        try
        {
    ...
    public async Task DisplayChangeHandling_DoesNotDisturbDeleteState_WhileReloading()
    {
        ...
        try
        {
            Guid deletingFenceId = Guid.NewGuid();
            bool beganDelete = manager.TryBeginDeleteForTesting(deletingFenceId);
            ...
            manager.IsDeletingForTesting(deletingFenceId).Should().BeTrue();
        }
        finally
        {
            manager.SetReloadingFromStateForTesting(false);
    ...
    public async Task DisplayChangeHandling_MultipleSignals_AreSerializedThroughQueue()
    {
        ...
        var coordinator = new MonitorLayoutCoordinator(
            ...
            captureCurrentSnapshot: () => new List<IVOEFences.Core.Models.FenceModel>(),
            ...
            loadLayoutSnapshotAsync: _ => Task.FromResult(new List<IVOEFences.Core.Models.FenceModel>()),
            getSettings: () => new IVOEFences.Core.Models.AppSettings
            {
                DetectMonitorConfigurationChanges = true,
                AutoSwapMisplacedFenceContents = false,
            },
