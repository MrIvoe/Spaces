...
/// <summary>
/// Full GDI rendering for a fence window — no WPF, no XAML.
/// GDI pens, brushes, and fonts are cached per theme (dark/light) and
/// reused across all fence windows; they are only recreated when the
...
/// </summary>
internal static class FenceRenderer
{
    ...
    private struct GdiResources
    {
        ...
        public IntPtr TitleFont;  // Segoe UI 13 pt — fence title text
    ...
    /// </summary>
    /// <param name="items">Fence items to render below the title bar; null = empty.</param>
    /// <param name="rolledUp">When true the fence is collapsed — skip item rendering.</param>
    /// <param name="positions">
    /// Pre-computed icon top-left positions from FenceIconLayout.
    ...
    public static void Paint(IntPtr hwnd, string title,
        IReadOnlyList<FenceItemModel>? items = null, bool rolledUp = false,
        Win32.POINT[]? positions = null, int iconSize = 48, string? searchQuery = null,
        FenceTabManager? tabManager = null, bool aiSuggested = false,
    ...
    private static void DrawFrame(IntPtr hdc, Win32.RECT client, string title,
        IReadOnlyList<FenceItemModel>? items, bool rolledUp, Win32.POINT[]? positions, int iconSize,
        string? searchQuery = null, FenceTabManager? tabManager = null, bool aiSuggested = false,
        bool showTitleBar = true, string? backgroundColorOverride = null, string? titleTextColorOverride = null,
        int scrollOffset = 0, bool snapFlash = false, bool isLocked = false,
        Guid? selectedItemId = null, Guid? hoveredItemId = null,
        bool isPortal = false, string portalViewMode = "Icons", int titleBarHeight = TitleBarHeight, int iconSpacing = 8, uint dpi = 96)
    {
        ...
        // Subtle AI highlight border for suggested fences.
        if (aiSuggested)
        {
        ...
        // Item grid (skipped when fence is rolled up)
    ...
    /// <summary>
    /// Draw fence items as an icon grid below the title bar.
    /// When <paramref name="positions"/> is provided (pre-computed by
    /// <see cref="FenceIconLayout"/>) the renderer skips its own layout
    ...
    /// <summary>
    /// Draw fence items as an icon grid below the title bar.
    /// When positions is provided (pre-computed by FenceIconLayout) the renderer skips
    ...
    private static void DrawItems(
        ...
        IReadOnlyList<FenceItemModel> items, bool dark, GdiResources res,
        ...
        Guid? selectedItemId, Guid? hoveredItemId)
    {
        ...
        for (int i = 0; i < items.Count; i++)
        {
            ...
            if (cy >= client.bottom)
                break;      // past the bottom of the fence — no more visible items
    ...
    private static void DrawPortalList(
        IntPtr hdc,
        Win32.RECT client,
        IReadOnlyList<FenceItemModel> items,
        bool dark,
        GdiResources res,
        int iconSize,
        int scrollOffset,
