...
internal sealed class HotkeyCoordinator : IDisposable
{
        ...
        Action commandPalette)
    {
        ...
        Register(1, Win32.MOD_CTRL | Win32.MOD_WIN, Win32.VK_N, "CreateFence", createFence);
        ...
        if (TryParseHotkey(settings.ToggleHotkey, out uint toggleMods, out uint toggleVk))
            Register(7, toggleMods, toggleVk, "ToggleAllFencesVisibility", toggleAllVisibility);
        else
            Register(7, Win32.MOD_WIN, Win32.VK_SPACE, "ToggleAllFencesVisibility", toggleAllVisibility);

        if (TryParseHotkey(settings.SearchHotkey, out uint searchMods, out uint searchVk))
            Register(8, searchMods, searchVk, "SearchAcrossFences", searchAcrossFences);
        else
            Register(8, Win32.MOD_WIN, Win32.VK_F, "SearchAcrossFences", searchAcrossFences);
