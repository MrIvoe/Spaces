...
/// Explorer restarts. Uses a live window provider (set by FenceManager) to
/// iterate current fence windows instead of accumulating stale callbacks.
/// </summary>
internal sealed class DesktopHost
{
    ...
    /// <summary>
    /// Set by FenceManager at startup. Returns all live FenceWindow instances
    ...
    // very frequently (WM_EXITSIZEMOVE, FenceManager startup).  Invalidated
    ...
    /// Rediscovers WorkerW and re-anchors all live fence windows.
    /// </summary>
    public void OnExplorerRestarted()
    {
        ...
        Log.Information(
            "DesktopHost: Explorer restarted — re-anchoring {TargetCount} fence target(s) using handle={Handle:X}; WorkerW={WorkerW:X}; desktop ListView={ListView:X}",
            targetCount,
            reAnchorHandle,
            _workerW,
            listView);
