...
/// and fence layout save/restore across monitor configurations.
...
internal sealed class MonitorLayoutCoordinator
{
    ...
    private readonly Func<List<FenceModel>> _captureCurrentSnapshot;
    private readonly Func<IEnumerable<FenceModel>, Task> _replaceAllAsync;
    private readonly Func<string, List<FenceModel>, Task> _saveLayoutSnapshotAsync;
    private readonly Func<string, Task<List<FenceModel>>> _loadLayoutSnapshotAsync;
    ...
    public MonitorLayoutCoordinator(FenceManager fences)
        : this(
            () => fences.IsReloadingFromState,
            fences.ReloadFromStateAsync,
            () => FenceStateService.Instance.Fences.ToList(),
            fencesToReplace => FenceStateService.Instance.ReplaceAllAsync(fencesToReplace),
            (configHash, fencesToSave) => FenceRepository.Instance.SaveLayoutSnapshotAsync(configHash, fencesToSave),
            configHash => FenceRepository.Instance.LoadLayoutSnapshotAsync(configHash),
    ...
    internal MonitorLayoutCoordinator(
        ...
        Func<List<FenceModel>> captureCurrentSnapshot,
        Func<IEnumerable<FenceModel>, Task> replaceAllAsync,
        Func<string, List<FenceModel>, Task> saveLayoutSnapshotAsync,
        Func<string, Task<List<FenceModel>>> loadLayoutSnapshotAsync,
    ...
    private async Task HandleDisplayChangeAsync()
    {
        try
        {
            ...
            {
                Log.Debug("MonitorLayoutCoordinator: display-change handling skipped while fence reload is active");
                return;
            }
            ...
            List<FenceModel> currentSnapshot = _captureCurrentSnapshot();
            await _saveLayoutSnapshotAsync(oldHash, currentSnapshot).ConfigureAwait(false);

            Log.Information(
                "MonitorLayoutCoordinator: monitor config changed old={OldHash} new={NewHash}; saved snapshot with {FenceCount} fences; monitors={Monitors}",
                oldHash, newHash, currentSnapshot.Count, string.Join(" | ", monitors));

            if (!settings.AutoSwapMisplacedFenceContents)
                return;

            List<FenceModel> restored = await _loadLayoutSnapshotAsync(newHash).ConfigureAwait(false);
            if (restored.Count == 0)
            {
            ...
            Log.Information("MonitorLayoutCoordinator: restored {FenceCount} fences for monitor config {Hash}", restored.Count, newHash);
        }
        catch (Exception ex)
        {
    ...
    public static void LogStartupDiagnostics()
    {
        try
        {
            ...
            Log.Information(
                ...
                settings.AutoSwapMisplacedFenceContents,
