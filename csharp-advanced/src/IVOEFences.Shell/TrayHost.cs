...
internal sealed class TrayHost
{
    ...
    // Raised when the user clicks "New Fence"
    public event Action? NewFenceRequested;
    // Raised when the user left-clicks and action is ToggleFences
    public event Action? ToggleFencesRequested;
    ...
    public TrayHost()
    {
        _menuCommandRoutes = new Dictionary<uint, Action>
        {
            ...
            [Shell32.CMD_NEW_FENCE] = () =>
            {
                Log.Information("TrayHost: New Fence selected");
                NewFenceRequested?.Invoke();
            },
            [Shell32.CMD_TOGGLE_FENCES] = () =>
            {
                Log.Information("TrayHost: Toggle Fences selected");
                ToggleFencesRequested?.Invoke();
            },
        };

        _leftClickRoutes = new Dictionary<string, Action>(StringComparer.OrdinalIgnoreCase)
        {
            ["ToggleFences"] = () => ToggleFencesRequested?.Invoke(),
    ...
    private void LoadIcon()
    {
        ...
        string iconPath = Path.Combine(AppContext.BaseDirectory, "Resources", "IVOEFences.ico");
    ...
    private void ShowContextMenu()
    {
        ...
        Shell32.AppendMenu(hMenu, Shell32.MF_STRING, (UIntPtr)Shell32.CMD_TOGGLE_FENCES, "Toggle Fences");
        Shell32.AppendMenu(hMenu, Shell32.MF_STRING, (UIntPtr)Shell32.CMD_NEW_FENCE, "New Fence");
