...
/// <summary>
/// Manages fence layouts per unique monitor configuration.
...
public class MonitorConfigurationManager
{
    ...
    /// <summary>
    /// Save current fence layouts with the given config hash.
    /// Called before switching to ensure previous config is preserved.
    /// </summary>
    public async Task SaveLayoutForConfigurationAsync(string configHash, List<Models.FenceModel> fences)
    {
        try
        {
            await _repository.SaveLayoutSnapshotAsync(configHash, fences).ConfigureAwait(false);
            Log.Information("Saved layout for config {Hash} with {FenceCount} fences", 
                configHash, fences.Count);
        }
        catch (Exception ex)
        {
    ...
    /// <summary>
    /// Load fence layouts for the specified monitor configuration.
    /// Returns empty list if config has never been seen before.
    /// </summary>
    public async Task<List<Models.FenceModel>> LoadLayoutForConfigurationAsync(string configHash)
    {
        try
        {
            var fences = await _repository.LoadLayoutSnapshotAsync(configHash).ConfigureAwait(false);
            Log.Information("Loaded {FenceCount} fences for config {Hash}", fences.Count, configHash);
            return fences;
        }
        catch (Exception ex)
        {
            Log.Error(ex, "Failed to load layout for configuration {Hash}", configHash);
            return new List<Models.FenceModel>();
        }
    }
    ...
    /// </summary>
    public List<(string Hash, int FenceCount, DateTime LastActive)> GetSavedConfigurations()
    {
        try
        {
    ...
    /// </summary>
    public async Task OnMonitorConfigurationChangedAsync(List<Models.FenceModel> currentFences)
    {
        ...
        {
            await SaveLayoutForConfigurationAsync(oldHash, currentFences).ConfigureAwait(false);
        }
    ...
    /// <summary>
    /// Validate that all fences in the list are still on valid monitors.
    /// If a monitor was removed, optionally move fences to primary.
    /// </summary>
    public void ValidateFencesForCurrentMonitorConfig(List<Models.FenceModel> fences)
    {
        ...
        {
            if (!validMonitors.Contains(fence.MonitorDeviceName))
            {
                // Monitor no longer exists - move to primary
                var primary = Screen.PrimaryScreen?.DeviceName ?? "Primary";
                Log.Warning("Fence {FenceId} was on removed monitor {Monitor}, moving to {Primary}",
                    fence.Id, fence.MonitorDeviceName, primary);
                fence.MonitorDeviceName = primary;
                movedCount++;
            }
        }
        ...
        if (movedCount > 0)
        {
            Log.Information("Moved {Count} fences due to removed monitors", movedCount);
        }
    }
}
