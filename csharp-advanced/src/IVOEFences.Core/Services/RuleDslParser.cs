...
public sealed class RuleDslParser
{
    ...
    /// parse errors.  Rules map to fences by title; the caller is responsible
    /// for resolving fence titles to IDs after calling this method.
    /// </summary>
    ...
    public string Export(IEnumerable<SortRulesEngine.SortRule> rules)
    {
        ...
        foreach (var rule in rules)
        {
            ...
            // Note: target fence title is stored in the pattern for Custom; for others
            // the rule.TargetFenceId must be resolved by the caller.
            sb.AppendLine($"then move to \"(fence-id:{rule.TargetFenceId})\"");
    ...
    private static SortRulesEngine.SortRule? ParseBlock(string[] lines, int startLine, List<string> errors)
    {
        ...
        foreach (string line in lines)
        {
            ...
            {
                targetFenceTitle = ExtractQuoted(line.Substring(14).Trim());
                continue;
            }
        ...
        {
            var typed = TryParseTypedCondition(conditionLines[0], ruleName, targetFenceTitle, priority);
            if (typed != null)
                return typed;
        }
        ...
        {
            Name           = ruleName,
            TargetFenceTitle = targetFenceTitle,
    ...
    private static SortRulesEngine.SortRule? TryParseTypedCondition(
        string condition, string ruleName, string targetFenceTitle, int priority)
    {
        Match m;

        m = ExtEq.Match(condition);
        if (m.Success)
            return MakeRule(ruleName, targetFenceTitle, priority,
    ...
    private static SortRulesEngine.SortRule MakeRule(string name, string fenceTitle, int priority,
