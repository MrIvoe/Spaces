...
/// <summary>
/// Step 26: Manages Peek™ feature (Win+Space brings all fences above other windows).
/// Handles z-order manipulation, state persistence, and window activation.
/// 
/// Peek temporarily lifts all fence windows above application windows,
...
public sealed class PeekService
{
    ...
    /// <summary>
    /// Activates Peek mode: brings all fences above active window(s).
    /// </summary>
    public void ActivatePeek()
    {
        ...
        try
        {
            // Find all fence windows and bring to front
            var all = FenceStateService.Instance.Fences;
            ...
            {
                var fenceHwnd = FindFenceWindow(fence.Id);
                if (fenceHwnd != IntPtr.Zero)
                {
                    _savedZOrder[fenceHwnd] = GetZOrder(fenceHwnd);
                    // Bring fence to top (HWND_TOP = 0)
            ...
            {
                IsActive = true,
                FenceCount = _savedZOrder.Count,
                TimeoutMs = _peekTimeoutMs
            });

            Serilog.Log.Information("Peek activated: {Count} fences brought to front", _savedZOrder.Count);
        }
        catch (Exception ex)
        {
    ...
    private IntPtr FindFenceWindow(Guid fenceId)
    {
        // TODO: Implement window search by fence ID
        // For now, return zero (will be updated when FenceWindow integration is added)
        return IntPtr.Zero;
    }
