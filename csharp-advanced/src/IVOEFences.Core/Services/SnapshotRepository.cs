...
/// <summary>
/// Step 33: Snapshot management for saving and restoring entire fence layouts.
/// Snapshots capture all fence positions, sizes, items, and state at a point in time.
...
public sealed class SnapshotRepository
{
    ...
    {
        var baseDir = Path.GetDirectoryName(AppPaths.FencesConfig) ?? Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData);
        _snapshotDir = Path.Combine(baseDir, "snapshots");
        Directory.CreateDirectory(_snapshotDir);
    }

    /// <summary>
    /// Represents a saved snapshot of fence layouts.
    /// </summary>
        ...
        bool IsAutoBackup)
    {
        ...
        public int TotalItems => Fences.Sum(f => f.Items?.Count ?? 0);
    }
    ...
    public Snapshot CreateSnapshot(string name, string? description = null)
    {
        try
        {
            var fences = FenceStateService.Instance.Fences.ToList();
            ...
            Serilog.Log.Information("Snapshot created: {SnapshotName} ({FenceCount} fences, {ItemCount} items)",
                name, snapshot.FenceCount, snapshot.TotalItems);
    ...
    public Snapshot CreateAutoBackup()
    {
        ...
        var snapshot = new Snapshot(
            ...
            CreatedAt: DateTime.UtcNow,
            LastModifiedAt: DateTime.UtcNow,
            Fences: FenceStateService.Instance.Fences.ToList(),
            IsAutoBackup: true
        );
    ...
    /// <summary>
    /// Step 33: Restore fences from a snapshot.
    /// </summary>
    public async Task<bool> RestoreSnapshot(Guid snapshotId)
    {
        try
        {
            ...
            await FenceStateService.Instance.ReplaceAllAsync(snapshot.Fences).ConfigureAwait(false);
            Serilog.Log.Information("Snapshot restored: {SnapshotName} ({FenceCount} fences)",
                snapshot.Name, snapshot.FenceCount);
