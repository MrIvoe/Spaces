...
internal sealed class SettingsWindow
{
    ...
    {
        SpaceWindow.SpaceSortMode.Manual.ToString(),
        SpaceWindow.SpaceSortMode.Name.ToString(),
        SpaceWindow.SpaceSortMode.Type.ToString(),
        SpaceWindow.SpaceSortMode.DateModified.ToString(),
        SpaceWindow.SpaceSortMode.Usage.ToString(),
    };
    ...
    private static readonly string[] PerSpaceTitlebarModes = { "UseGlobal", "Visible", "Mouseover", "Hidden" };
    private static readonly string[] PerSpaceRollupModes = { "UseGlobal", "ClickToOpen", "HoverToOpen", "AlwaysExpanded", "Disabled" };
    private static readonly string[] PerSpaceSnapModes = { "UseGlobal", "Snap", "Free" };
    private static readonly string[] PerSpacePortalModes = { "UseGlobal", "Enabled", "Disabled" };
    private static readonly string[] PerSpaceBlurModes = { "UseGlobal", "Enabled", "Disabled" };
    private static readonly string[] LiveFolderViews = { "Icons", "List", "Details" };
    private static readonly string[] StandardSpaceDropModes = { "Reference", "MoveIntoSpaceStorage", "Shortcut" };
    private static readonly string[] TrayLeftClickActions = { "OpenSettings", "ToggleSpaces", "ShowMenu" };

    // ── Live mutable settings (edited in-window, committed on Save) ────────
    private bool _showSpacesOnStartup;
    private bool _hideDesktopOutsideSpaces;
    ...
    private string _defaultSortMode = SpaceWindow.SpaceSortMode.Name.ToString();
    private int _iconSize;
    private int _iconSpacing;
    private bool _autoResizeSpacesOnIconSizeChange;
    private bool _showSpaceTitles;
    private int _spaceOpacity;
    private bool _blurBackground;
    private int _glassStrength;
    private bool _enableAnimations;
    private bool _snapToScreenEdges;
    private bool _snapToOtherSpaces;
    private bool _snapToGrid;
    private int _interSpaceSpacing;
    ...
    private string _standardSpaceDropMode = "Reference";
    ...
    public static void ShowOrFocus(SpaceManager manager, IReadOnlyList<PluginSettingDefinition> pluginSettings)
    {
    ...
    private SettingsWindow(SpaceManager manager, IReadOnlyList<PluginSettingDefinition> pluginSettings)
    {
        _manager = manager;
    ...
    private void LoadFromSettings()
    {
        ...
        _showSpacesOnStartup = s.ShowSpacesOnStartup;
        _hideDesktopOutsideSpaces = s.HideDesktopIconsOutsideSpaces;
        ...
        _autoResizeSpacesOnIconSizeChange = s.AutoResizeSpacesOnIconSizeChange;
        _showSpaceTitles = s.ShowSpaceTitles;
        _spaceOpacity = s.SpaceOpacity;
        ...
        _snapToOtherSpaces = s.SnapToOtherSpaces;
        _snapToGrid = s.SnapToGrid;
        _interSpaceSpacing = Math.Clamp(s.InterSpaceSpacing, 0, 64);
        ...
        _standardSpaceDropMode = string.IsNullOrWhiteSpace(s.StandardSpaceDropMode) ? "Reference" : s.StandardSpaceDropMode;
    ...
    private void CommitToSettings()
    {
        ...
        int nextInterSpaceSpacing = Math.Clamp(_interSpaceSpacing, 0, 64);
        ...
        if (current.ShowSpacesOnStartup != _showSpacesOnStartup) changedKeys.Add(nameof(AppSettings.ShowSpacesOnStartup));
        if (current.HideDesktopIconsOutsideSpaces != _hideDesktopOutsideSpaces) changedKeys.Add(nameof(AppSettings.HideDesktopIconsOutsideSpaces));
        ...
        if (current.AutoResizeSpacesOnIconSizeChange != _autoResizeSpacesOnIconSizeChange) changedKeys.Add(nameof(AppSettings.AutoResizeSpacesOnIconSizeChange));
        if (current.ShowSpaceTitles != _showSpaceTitles) changedKeys.Add(nameof(AppSettings.ShowSpaceTitles));
        if (current.SpaceOpacity != _spaceOpacity) changedKeys.Add(nameof(AppSettings.SpaceOpacity));
        ...
        if (current.SnapToOtherSpaces != _snapToOtherSpaces) changedKeys.Add(nameof(AppSettings.SnapToOtherSpaces));
        if (current.SnapToGrid != _snapToGrid) changedKeys.Add(nameof(AppSettings.SnapToGrid));
        if (current.InterSpaceSpacing != nextInterSpaceSpacing) changedKeys.Add(nameof(AppSettings.InterSpaceSpacing));
        ...
        if (!string.Equals(current.StandardSpaceDropMode, _standardSpaceDropMode, StringComparison.Ordinal)) changedKeys.Add(nameof(AppSettings.StandardSpaceDropMode));
        ...
        {
            s.ShowSpacesOnStartup = _showSpacesOnStartup;
            s.HideDesktopIconsOutsideSpaces = _hideDesktopOutsideSpaces;
            ...
            s.AutoResizeSpacesOnIconSizeChange = _autoResizeSpacesOnIconSizeChange;
            s.ShowSpaceTitles = _showSpaceTitles;
            s.SpaceOpacity = _spaceOpacity;
            ...
            s.SnapToOtherSpaces = _snapToOtherSpaces;
            s.SnapToGrid = _snapToGrid;
            s.InterSpaceSpacing = nextInterSpaceSpacing;
            ...
            s.StandardSpaceDropMode = _standardSpaceDropMode;
    ...
    private SpaceWindow? GetSelectedSpace()
    {
        List<SpaceWindow> windows = _manager.Windows.ToList();
        if (windows.Count == 0)
            return null;

        SpaceWindow? selected = null;
        if (_selectedSpaceId.HasValue)
            selected = windows.FirstOrDefault(w => w.ModelId == _selectedSpaceId.Value);

        if (selected == null)
        {
            selected = windows[0];
            _selectedSpaceId = selected.ModelId;
        }
    ...
    private void PreviewSpaceSettings()
    {
        foreach (SpaceWindow window in _manager.Windows)
        {
    ...
    private void Paint(IntPtr hwnd)
    {
        ...
        switch (_activeTab)
        {
            ...
            case Tab.Hotkeys:
                ...
                break;
            case Tab.Spaces:
                DrawSpacesTab(hdc, semiboldFont, client.right, dark);
                break;
            case Tab.Rules:
    ...
    {
        string[] labels = { "General", "Hotkeys", "Spaces", "Rules", "Pages", "Plugins", "Advanced" };
    ...
    private void DrawGeneralTab(IntPtr hdc, int clientRight, bool dark)
    {
        ...
        y = DrawBoolRow(hdc, y, clientRight, "Show spaces on startup", _showSpacesOnStartup, () => _showSpacesOnStartup = !_showSpacesOnStartup, dark);
        ...
        y += 30;
        y = DrawBoolRow(hdc, y, clientRight, "Hide desktop originals while spaced", _hideDesktopOutsideSpaces, () => _hideDesktopOutsideSpaces = !_hideDesktopOutsideSpaces, dark);
    ...
    private void DrawHotkeysTab(IntPtr hdc, int clientRight, bool dark)
    {
        ...
        y = DrawChoiceRow(hdc, y, clientRight, "Toggle spaces hotkey", _toggleHotkey, () => _toggleHotkey = NextString(_toggleHotkey, new[] { "Win+Space", "Ctrl+Win+Space", "Ctrl+Win+T" }), dark);
    ...
    private void DrawSpacesTab(IntPtr hdc, IntPtr semiboldFont, int clientRight, bool dark)
    {
        int y = ScrolledContentTop;
        DrawSectionHeader(hdc, "Global Space Defaults", y - 26, clientRight, dark);
        ...
        y = DrawBoolRow(hdc, y, clientRight, "Auto-resize spaces when icon size changes", _autoResizeSpacesOnIconSizeChange,
            () => UpdateAndPreview(() => _autoResizeSpacesOnIconSizeChange = !_autoResizeSpacesOnIconSizeChange), dark);
        ...
        y = DrawBoolRow(hdc, y, clientRight, "Require click to open rollup spaces", _rollupRequiresClick,
            () => UpdateAndPreview(() =>
            {
        ...
        y = DrawBoolRow(hdc, y, clientRight, "Require hover to open rollup spaces", _rollupRequiresHover,
            () => UpdateAndPreview(() =>
            {
        ...
        y = DrawBoolRow(hdc, y, clientRight, "Show space titles", _showSpaceTitles, () => UpdateAndPreview(() =>
            _showSpaceTitles = !_showSpaceTitles), dark);
        y = DrawChoiceRow(hdc, y, clientRight, "Space opacity", $"{_spaceOpacity}%", () => UpdateAndPreview(() =>
            _spaceOpacity = NextInt(_spaceOpacity, OpacityLevels)), dark);
        ...
        Win32.SetBkMode(hdc, Win32.TRANSPARENT);
        Win32.SetTextColor(hdc, ClrTextMuted(dark));
        Win32.DrawText(hdc, "Per-space controls", -1, ref sectionTitle, Win32.DT_LEFT | Win32.DT_VCENTER | Win32.DT_SINGLELINE);
        ...
        if (windows.Count == 0)
        {
            ...
            Win32.SetTextColor(hdc, ClrTextMuted(dark));
            Win32.DrawText(hdc, "No spaces are currently active", -1, ref emptyRect, Win32.DT_LEFT | Win32.DT_SINGLELINE | Win32.DT_VCENTER);
            return;
        }

        if (!_selectedSpaceId.HasValue || windows.All(w => w.ModelId != _selectedSpaceId.Value))
            _selectedSpaceId = windows[0].ModelId;

        int rowY = listTop + 6;
        for (int i = 0; i < windows.Count && i < 4; i++)
        {
            SpaceWindow win = windows[i];
            bool selected = win.ModelId == _selectedSpaceId.Value;
            ...
            Guid id = win.ModelId;
            AddHitTarget(rowRect, () => _selectedSpaceId = id);
            rowY += 31;
        }

        SpaceWindow? selectedSpace = GetSelectedSpace();
        if (selectedSpace == null)
            return;

        int detailsY = listTop + listHeight + 8;
        detailsY = DrawChoiceRow(hdc, detailsY, clientRight, "Selected space sort", selectedSpace.GetSortMode().ToString(), () =>
        {
            string next = NextString(selectedSpace.GetSortMode().ToString(), SortModes);
            if (Enum.TryParse(next, ignoreCase: true, out SpaceWindow.SpaceSortMode mode))
                selectedSpace.SortIcons(mode, animate: true);
        }, dark, trackSettingChange: false);

        detailsY = DrawChoiceRow(hdc, detailsY, clientRight, "Selected space icon size", $"{selectedSpace.GetIconSize()}px", () =>
        {
            int next = NextInt(selectedSpace.GetIconSize(), IconSizes);
            selectedSpace.SetIconSize(next, notifyManager: true);
        }, dark, trackSettingChange: false);

        detailsY = DrawBoolRow(hdc, detailsY, clientRight, "Selected space collapsed", selectedSpace.IsCollapsed,
            () => selectedSpace.ToggleCollapseExpand(), dark, trackSettingChange: false);

        detailsY = DrawBoolRow(hdc, detailsY, clientRight, "Selected space locked", selectedSpace.IsLocked(),
            selectedSpace.ToggleLocked, dark, trackSettingChange: false);

        detailsY = DrawChoiceRow(hdc, detailsY, clientRight, "Selected titlebar mode", selectedSpace.GetPerSpaceTitlebarModeOverride(), () =>
        {
            string next = NextString(selectedSpace.GetPerSpaceTitlebarModeOverride(), PerSpaceTitlebarModes);
            selectedSpace.SetPerSpaceTitlebarModeOverride(next);
            SettingsManager.Instance.NotifySpaceChanged(selectedSpace.ModelId, "space.titlebar.mode");
            selectedSpace.InvalidateContent();
        }, dark, trackSettingChange: false);

        detailsY = DrawChoiceRow(hdc, detailsY, clientRight, "Selected roll-up mode", selectedSpace.GetPerSpaceRollupModeOverride(), () =>
        {
            string next = NextString(selectedSpace.GetPerSpaceRollupModeOverride(), PerSpaceRollupModes);
            selectedSpace.SetPerSpaceRollupModeOverride(next);
            SettingsManager.Instance.NotifySpaceChanged(selectedSpace.ModelId, "space.rollup.mode");
        }, dark, trackSettingChange: false);

        detailsY = DrawChoiceRow(hdc, detailsY, clientRight, "Selected snap mode", selectedSpace.GetPerSpaceSnapModeOverride(), () =>
        {
            string next = NextString(selectedSpace.GetPerSpaceSnapModeOverride(), PerSpaceSnapModes);
            selectedSpace.SetPerSpaceSnapModeOverride(next);
            SettingsManager.Instance.NotifySpaceChanged(selectedSpace.ModelId, "space.position.mode");
        }, dark, trackSettingChange: false);

        detailsY = DrawChoiceRow(hdc, detailsY, clientRight, "Selected portal mode", selectedSpace.GetPerSpacePortalOverride(), () =>
        {
            string next = NextString(selectedSpace.GetPerSpacePortalOverride(), PerSpacePortalModes);
            selectedSpace.SetPerSpacePortalOverride(next);
            SettingsManager.Instance.NotifySpaceChanged(selectedSpace.ModelId, "space.portal.mode");
        }, dark, trackSettingChange: false);

        detailsY = DrawChoiceRow(hdc, detailsY, clientRight, "Selected glass panel", selectedSpace.GetPerSpaceBlurOverride(), () =>
        {
            string next = NextString(selectedSpace.GetPerSpaceBlurOverride(), PerSpaceBlurModes);
            selectedSpace.SetPerSpaceBlurOverride(next);
            SettingsManager.Instance.NotifySpaceChanged(selectedSpace.ModelId, "space.blur.enabled");
        }, dark, trackSettingChange: false);

        detailsY = DrawChoiceRow(hdc, detailsY, clientRight, "Selected live folder view", selectedSpace.GetPerSpacePortalViewOverride(), () =>
        {
            string next = NextString(selectedSpace.GetPerSpacePortalViewOverride(), LiveFolderViews);
            selectedSpace.SetPerSpacePortalViewOverride(next);
            SettingsManager.Instance.NotifySpaceChanged(selectedSpace.ModelId, "space.portal.view");
            selectedSpace.InvalidateContent();
        }, dark, trackSettingChange: false);

        detailsY = DrawChoiceRow(hdc, detailsY, clientRight, "Selected space color", selectedSpace.GetSpaceColorPresetLabel(),
            selectedSpace.CycleSpaceColorOverride, dark, trackSettingChange: false);

        detailsY = DrawChoiceRow(hdc, detailsY, clientRight, "Selected bar mode", _manager.GetBarModeSummary(selectedSpace.ModelId), () =>
        {
            bool changed = _manager.ToggleBarMode(selectedSpace.ModelId);
            _statusText = changed
                ? $"Bar mode: {_manager.GetBarModeSummary(selectedSpace.ModelId)}"
        ...
        {
            bool changed = _manager.CycleBarDockEdge(selectedSpace.ModelId);
            _statusText = changed
                ? $"Bar mode: {_manager.GetBarModeSummary(selectedSpace.ModelId)}"
        ...
        {
            bool changed = _manager.CycleBarThickness(selectedSpace.ModelId);
            _statusText = changed
                ? $"Bar mode: {_manager.GetBarModeSummary(selectedSpace.ModelId)}"
                : "Enable bar mode first";
            SettingsManager.Instance.NotifyRuntimeRefresh();
        ...
        detailsY = DrawChoiceRow(hdc, detailsY, clientRight, "Tab group", _manager.GetTabGroupSummary(selectedSpace.ModelId), () =>
        {
            List<SpaceWindow> windows = _manager.Windows.Where(w => w.ModelId != selectedSpace.ModelId).ToList();
            if (windows.Count == 0)
            {
                _statusText = "Need at least two spaces to create tabs";
                return;
            }

            SpaceWindow target = windows[0];
            bool merged = _manager.MergeSpacesIntoTabGroup(selectedSpace.ModelId, target.ModelId);
            _statusText = merged
                ? $"Merged {selectedSpace.Title} + {target.Title} into tab group"
        ...
        {
            bool switched = _manager.SwitchTabForSpace(selectedSpace.ModelId, +1);
            _statusText = switched ? "Switched to next tab" : "Space is not in a tab group";
            SettingsManager.Instance.NotifyRuntimeRefresh();
        }, dark);
        ...
        {
            bool switched = _manager.SwitchTabForSpace(selectedSpace.ModelId, -1);
            _statusText = switched ? "Switched to previous tab" : "Space is not in a tab group";
            SettingsManager.Instance.NotifyRuntimeRefresh();
        }, dark);

        detailsY = DrawActionRow(hdc, detailsY, clientRight, "Tab group", "Dissolve tab group", () =>
        {
            bool dissolved = _manager.DissolveTabGroupForSpace(selectedSpace.ModelId);
            _statusText = dissolved ? "Tab group dissolved" : "Space is not in a tab group";
            SettingsManager.Instance.NotifyRuntimeRefresh();
        }, dark);

        _ = DrawActionRow(hdc, detailsY, clientRight, "Selected space name", "Rename", () =>
        {
            string? input = Win32InputDialog.Show(_hwnd, "Enter a new name for the selected space.", "Rename Space", selectedSpace.Title);
            if (!string.IsNullOrWhiteSpace(input))
                selectedSpace.Rename(input);
        }, dark);
    }
    ...
    private void DrawDesktopPagesTab(IntPtr hdc, int clientRight, bool dark)
    {
        ...
        if (selectedSpace == null)
        {
            _ = DrawActionRow(hdc, y, clientRight, "Selected space", "No space selected", () => { }, dark);
            return;
        }

        y = DrawChoiceRow(hdc, y, clientRight, "Selected space", selectedSpace.Title, CycleSelectedSpace, dark, trackSettingChange: false);
        y = DrawChoiceRow(hdc, y, clientRight, "Selected space page", $"Page {selectedSpace.GetPageIndex() + 1}", () =>
        {
            int totalPages = Math.Max(1, PageService.Instance.TotalPages);
            int targetPage = (selectedSpace.GetPageIndex() + 1) % totalPages;
            if (PageService.Instance.MoveSpaceToPage(selectedSpace.ModelId, targetPage))
            {
                RefreshDesktopPageState();
                _statusText = $"Moved {selectedSpace.Title} to page {targetPage + 1}";
                SettingsManager.Instance.NotifyRuntimeRefresh();
            }
            else
            {
                _statusText = $"Failed to move {selectedSpace.Title}";
            }
        }, dark, trackSettingChange: false);

        _ = DrawActionRow(hdc, y, clientRight, "Selected space", "Move to active page", () =>
        {
            if (PageService.Instance.MoveSpaceToPage(selectedSpace.ModelId, _currentDesktopPage))
            {
                _statusText = $"Moved {selectedSpace.Title} to active page {_currentDesktopPage + 1}";
                SettingsManager.Instance.NotifyRuntimeRefresh();
            }
            else
            {
    ...
    private void DrawRulesTab(IntPtr hdc, int clientRight, bool dark)
    {
        ...
        y = DrawBoolRow(hdc, y, clientRight, "Snap to other spaces", _snapToOtherSpaces, () => UpdateAndPreview(() => _snapToOtherSpaces = !_snapToOtherSpaces), dark);
        y = DrawBoolRow(hdc, y, clientRight, "Snap to grid", _snapToGrid, () => UpdateAndPreview(() => _snapToGrid = !_snapToGrid), dark);
        y = DrawChoiceRow(hdc, y, clientRight, "Inter-space spacing", $"{_interSpaceSpacing}px", () => UpdateAndPreview(() =>
            _interSpaceSpacing = NextInt(_interSpaceSpacing, SpacingOptions)), dark);
        ...
        y += 30;
        y = DrawChoiceRow(hdc, y, clientRight, "Standard space drop behavior", _standardSpaceDropMode,
            () => _standardSpaceDropMode = NextString(_standardSpaceDropMode, StandardSpaceDropModes), dark);
        ...
        if (selectedSpace == null)
        {
            _ = DrawActionRow(hdc, y, clientRight, "Space-specific rules", "No space selected", () => { }, dark);
            return;
        }

        y = DrawChoiceRow(hdc, y, clientRight, "Selected space", selectedSpace.Title, CycleSelectedSpace, dark, trackSettingChange: false);

        y = DrawActionRow(hdc, y, clientRight, "Quick preset", "Clear desktop to selected space", () =>
        {
            int moved = _manager.ApplyCatchAllPresetRule(selectedSpace.ModelId);
            _statusText = $"Catch-all preset applied to {selectedSpace.Title} ({moved} moved)";
        }, dark);

        y = DrawActionRow(hdc, y, clientRight,
            "Include rules",
            string.IsNullOrWhiteSpace(selectedSpace.GetIncludeRulesText()) ? "Edit" : selectedSpace.GetIncludeRulesText(),
            () =>
            {
                string? input = Win32InputDialog.Show(
                    ...
                    "Space Include Rules",
                    selectedSpace.GetIncludeRulesText());
                if (input != null)
                {
                    selectedSpace.SetIncludeRulesText(input);
                    int moved = _manager.ApplyAutoPlacementRulesLive();
        ...
        _ = DrawActionRow(hdc, y, clientRight,
            "Exclude rules",
            string.IsNullOrWhiteSpace(selectedSpace.GetExcludeRulesText()) ? "Edit" : selectedSpace.GetExcludeRulesText(),
            () =>
            {
                string? input = Win32InputDialog.Show(
                    _hwnd,
                    ...
                    "Space Exclude Rules",
                    selectedSpace.GetExcludeRulesText());
                if (input != null)
                {
                    selectedSpace.SetExcludeRulesText(input);
                    int moved = _manager.ApplyAutoPlacementRulesLive();
    ...
    private void CycleSelectedSpace()
    {
        List<SpaceWindow> windows = _manager.Windows.ToList();
        if (windows.Count == 0)
            return;
        ...
        int currentIndex = windows.FindIndex(w => w.ModelId == _selectedSpaceId.Value);
        if (currentIndex < 0)
        {
            _selectedSpaceId = windows[0].ModelId;
            return;
        ...
        _selectedSpaceId = windows[nextIndex].ModelId;
    }
