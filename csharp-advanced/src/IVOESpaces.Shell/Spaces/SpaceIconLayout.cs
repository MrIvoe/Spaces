...
/// <summary>
/// Pre-computes the pixel position of every icon in the space grid and caches the
/// result until the space is resized.  Keeping the layout out of WM_PAINT means
...
/// </summary>
internal sealed class SpaceIconLayout
{
    // Layout parameters — kept in sync with SpaceRenderer's padding constants
    ...
    /// </summary>
    public Win32.POINT[] GetPositions(IReadOnlyList<SpaceItemModel> items, int spaceWidth, int iconSize, int iconSpacing, int titleBarHeight)
    {
        if (_valid
        ...
        _positions  = Compute(items, spaceWidth, iconSize, iconSpacing, titleBarHeight);
        _spaceWidth = spaceWidth;
        _iconSize   = iconSize;
        _iconSpacing = iconSpacing;
        _titleBarHeight = titleBarHeight;
    ...
    private static Win32.POINT[] Compute(IReadOnlyList<SpaceItemModel> items, int spaceWidth, int iconSize, int iconSpacing, int titleBarHeight)
    {
        ...
        int cols     = Math.Max(1, (spaceWidth - PadX) / cellW);
    ...
    private static bool HasStableStoredGrid(IReadOnlyList<SpaceItemModel> items)
    {
        if (items.Count < 2)
            return false;
    ...
    /// <summary>
    /// Return the index of the item at space-local coordinates (lx, ly), or -1.
    /// Used for left-click / drag hit-testing without scanning items in WM_LBUTTONDOWN.
    /// </summary>
    public int HitTestItem(int lx, int ly, int itemCount, int spaceWidth, int spaceHeight, int iconSize, int iconSpacing, int scrollOffset, int titleBarHeight)
    {
        int cellH    = GetCellHeight(iconSize, iconSpacing);
        int cellW    = GetCellWidth(iconSize, iconSpacing);

        if (_positions == null || !_valid) return -1;
        if (ly <= titleBarHeight || ly >= spaceHeight) return -1;
    ...
    public int GetContentHeight(int itemCount, int spaceWidth, int iconSize, int iconSpacing, int titleBarHeight)
    {
        int verticalGap = Math.Max(iconSpacing / 2, PadY);
        int contentPadY = titleBarHeight + verticalGap;
        if (itemCount <= 0)
