...
/// types implementing <see cref="ISpacePlugin"/>, loads them, and manages their lifecycle.
///
/// Plugin DLLs must:
///   - End in ".Plugin.dll" (e.g. "MyTool.Plugin.dll")
///   - Contain at least one non-abstract class implementing <see cref="ISpacePlugin"/>
///   - Reference IVOESpaces.Core but NOT IVOESpaces.Shell (to avoid circular deps)
/// </summary>
internal sealed class PluginLoader : IDisposable
{
    private readonly List<ISpacePlugin> _loaded = new();
    ...
    public PluginLoader(Func<string, CreateSpaceResult>? createSpace = null)
    {
        ...
        _context = new PluginContext(createSpace);
    }

    public IReadOnlyList<ISpacePlugin> Plugins => _loaded;
        ...
        try
        {
            ...
            try
            {
                pluginTypes = asm.GetTypes()
                    .Where(t => typeof(ISpacePlugin).IsAssignableFrom(t)
                             && !t.IsAbstract
    ...
    private void LoadPlugin(Type type, string dllPath)
    {
        try
        {
            var plugin = (ISpacePlugin)Activator.CreateInstance(type)!;
    ...
    private sealed class PluginContext : IPluginContext
    {
        ...
        private readonly Func<string, CreateSpaceResult>? _createSpace;
        private readonly PluginManagerService _pluginManager;
        private readonly PluginUpdateService _pluginUpdateService;
        private readonly ThemeService _themeService;

        public PluginContext(Func<string, CreateSpaceResult>? createSpace)
        {
        ...
        public IReadOnlyList<SpaceModel> Spaces =>
            SpaceStateService.Instance.Spaces;
        ...
        public Guid CreateSpace(string title)
        {
            if (_createSpace == null)
            {
                string message = "PluginContext: CreateSpace callback unavailable; refusing persist-only fallback";
                Serilog.Log.Error("{Message} title={Title}", message, title);
                throw new InvalidOperationException(message);
            }

            CreateSpaceResult result = _createSpace(title);
            if (result.SpaceId == Guid.Empty || !result.WindowCreated || result.WindowHandle == IntPtr.Zero)
            {
                string message =
                    $"PluginContext: CreateSpace failed to materialize runtime space title='{title}' id='{result.SpaceId}' windowCreated={result.WindowCreated} hwnd=0x{result.WindowHandle.ToInt64():X}";
                Serilog.Log.Error("{Message}", message);
                throw new InvalidOperationException(message);
            }

            Serilog.Log.Information(
                "PluginContext: CreateSpace materialized title='{Title}' id='{SpaceId}' hwnd=0x{Hwnd:X}",
                title,
                result.SpaceId,
        ...
        public bool MoveFileToSpace(string filePath, string spaceTitle)
        {
            if (string.IsNullOrWhiteSpace(filePath) || string.IsNullOrWhiteSpace(spaceTitle))
                return false;

            SpaceModel? space = SpaceStateService.Instance.Spaces.FirstOrDefault(f =>
                f.Type == SpaceType.Standard &&
                string.Equals(f.Title, spaceTitle, StringComparison.OrdinalIgnoreCase));
            ...
            return DragDropPolicyService.Instance.ImportIntoSpace(space, filePath) != null;
        }
