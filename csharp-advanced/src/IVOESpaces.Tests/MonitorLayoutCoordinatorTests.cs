...
public class MonitorLayoutCoordinatorTests
{
    ...
    public async Task DisplayChange_WithSavedSnapshot_RestoresStateThenReloadsInOrder()
    {
        ...
        var restored = new List<SpaceModel>
        {
            new()
            {
                Id = Guid.NewGuid(),
        ...
        var coordinator = new MonitorLayoutCoordinator(
            ...
            {
                steps.Add("capture");
                return new List<SpaceModel> { new() { Id = Guid.NewGuid(), Title = "Current", WidthFraction = 0.2, HeightFraction = 0.2 } };
            },
            replaceAllAsync: spaces =>
            {
                steps.Add($"replace:{spaces.Count()}");
                return Task.CompletedTask;
            },
            saveLayoutSnapshotAsync: (configHash, spacesToSave) =>
            {
                steps.Add($"save:{configHash}:{spacesToSave.Count}");
                return Task.CompletedTask;
            },
    ...
    public async Task OverlappingDisplayChanges_RunSingleRestoreSequence()
    {
        ...
        var coordinator = new MonitorLayoutCoordinator(
            ...
            captureCurrentSnapshot: () => new List<SpaceModel> { new() { Id = Guid.NewGuid(), WidthFraction = 0.2, HeightFraction = 0.2 } },
    ...
    {
        List<SpaceModel>? replaced = null;
        bool usedFallback = false;
        var planner = new SpaceRestorePlacementPlanner((string _, out Win32.RECT workArea) =>
        {
            workArea = default;
            return false;
        });
        ...
        var coordinator = new MonitorLayoutCoordinator(
            ...
            reloadFromStateAsync: () =>
            {
                SpaceModel restoredSpace = replaced!.Single();
                SpaceRestorePlacementPlanner.PlacementPlan plan = planner.Plan(
                    restoredSpace,
            ...
            replaceAllAsync: spaces =>
            {
                replaced = spaces.ToList();
                return Task.CompletedTask;
            },
            saveLayoutSnapshotAsync: (_, _) => Task.CompletedTask,
            loadLayoutSnapshotAsync: _ => Task.FromResult(new List<SpaceModel>
            {
                new()
                {
                    Id = Guid.NewGuid(),
    ...
    public async Task DisplayChange_WithUnchangedTopology_DoesNotPersistOrReload()
    {
        ...
        var coordinator = new MonitorLayoutCoordinator(
            ...
            captureCurrentSnapshot: () => new List<SpaceModel>(),
            replaceAllAsync: _ => Task.CompletedTask,
            saveLayoutSnapshotAsync: (_, _) =>
            {
                Interlocked.Increment(ref saveCalls);
            ...
            {
                Interlocked.Increment(ref loadCalls);
                return Task.FromResult(new List<SpaceModel>());
            },
            getSettings: () => new AppSettings
            {
                DetectMonitorConfigurationChanges = true,
