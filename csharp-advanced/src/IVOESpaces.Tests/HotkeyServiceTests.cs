...
public class HotkeyServiceTests
{
    private static bool TryParse(string hotkey, out uint modifiers, out uint virtualKey)
    {
        var shellAssembly = AppDomain.CurrentDomain.GetAssemblies()
            .FirstOrDefault(a => string.Equals(a.GetName().Name, "IVOESpaces", StringComparison.Ordinal))
            ?? AppDomain.CurrentDomain.Load("IVOESpaces");

        var type = shellAssembly.GetType("IVOESpaces.Shell.HotkeyCoordinator")!;
