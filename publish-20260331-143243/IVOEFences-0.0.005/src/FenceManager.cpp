...
void FenceManager::CreateFenceFromData(const FenceData& data) {
    ...
    params.backingFolder = data.backingFolder;
...
void FenceManager::RefreshFenceItems(FenceWindow& fence, FenceData* record) {
    ...
    if (record != nullptr && !record->members.empty()) {
        labels.reserve(record->members.size());
        for (const DesktopItemRef& member : record->members) {
            if (!member.displayName.empty()) {
                labels.push_back(member.displayName);
            } else if (!member.sourcePath.empty()) {
                labels.push_back(std::filesystem::path(member.sourcePath).filename().wstring());
            }
        }
    }

    if (labels.empty() && !fence.GetBackingFolder().empty()) {
        DesktopItemService itemService;
        itemService.Initialize();
        const std::vector<DesktopItemRef> members = itemService.BuildLegacyFolderMembership(fence.GetBackingFolder());
        labels.reserve(members.size());
        for (const DesktopItemRef& member : members) {
            labels.push_back(member.displayName);
        }

        if (record != nullptr && !members.empty()) {
            record->members = members;
        }
    }

    fence.SetItemLabels(std::move(labels));
}
...
void FenceManager::HandleDroppedFiles(int fenceId, const std::vector<std::wstring>& paths) {
    ...
    std::wstring destDir = EnsureFenceBackingFolder(fence);
    if (destDir.empty()) {
        return;
    }
    ...
    for (const std::wstring& source : paths) {
        ...
        if (policy == DropPolicy::Move && UserIntegrityLauncher::IsCurrentProcessElevated()) {
            done = UserIntegrityLauncher::LaunchMoveWithUserIntegrity(source, destDir);
        }

        if (!done) {
            if (policy == DropPolicy::Copy) {
                done = PathMove::CopyPathToDirectory(source, destDir, nullptr);
            } else {
                done = PathMove::MovePathToDirectory(source, destDir, nullptr);
            }
        }
    ...
    if (record != nullptr) {
        DesktopItemService itemService;
        itemService.Initialize();
        record->backingFolder = destDir;
        record->members = itemService.BuildLegacyFolderMembership(destDir);
    }
