...
constexpr UINT ID_TRAY_NEW_FENCE = 1001;
...
std::wstring PickFolderPath(HWND owner) {
    ...
    browseInfo.lpszTitle = L"Select a folder for Portal Fence";
...
TrayIcon::TrayIcon(HINSTANCE instance, FenceManager* manager)
...
bool TrayIcon::Initialize() {
    ...
    std::wstring trayTip = std::wstring(L"IVOE Fences ") + AppVersion::kCurrent;
...
void TrayIcon::ShowMenu() {
    ...
    AppendMenuW(menu, MF_STRING, ID_TRAY_NEW_PORTAL_FENCE, L"New Portal Fence...");
    ...
    if (cmd == ID_TRAY_NEW_FENCE) {
        m_manager->CreateFenceAt(pt);
    } else if (cmd == ID_TRAY_NEW_PORTAL_FENCE) {
        const std::wstring folder = PickFolderPath(m_hwnd);
        if (!folder.empty()) {
            m_manager->CreatePortalFenceAt(pt, folder);
        }
    } else if (cmd == ID_TRAY_OPEN_FOLDER) {
        m_manager->OpenFirstFenceBackingFolder();
